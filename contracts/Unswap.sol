...
contract  Unswap is ERC20{
  address  public token1Address;
  address  public token2Address;

    constructor(address _token1Address,address _token2Address)ERC20("unswap" , "US"){
      
      require(_token1Address != address(0) && _token1Address != address(0),"invalid address");
...
function removeLiquidity(uint256 liquidity)public returns(uint256,uint256){
  ...
  function  getToken1Amount(uint256 _token1Sold)public view returns(uint256){
    ...
    function getToken2Amount(uint256 _token2Sold)public view returns(uint256){
...
//swap
  function Token1ToToken2Swap(uint256 _token1Sold)public {
   uint256 token1Reserve;
    ...
    uint256 _minEth = token2Bought-token2Bought*CaculateToken2Slippage(token2Bought);//滑点计算
