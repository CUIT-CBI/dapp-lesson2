...
contract UniswapPair is IUniswapPair, UniswapERC20 {
    ...
    address override public factory; //工厂地址
    address override public token0; //token0地址
    address override public token1; //token1地址
    ...
    returns (bool feeOn)
    {
        ...
        //如果feeTo不等于0地址,feeOn等于true否则为false 如果未设置feeTo地址就不会收取任何费用
     ...
     */
    // 添加流动性 加入储备量
    // 流程 add Liquidity -》 getPair -》create pair -》 isFirstAdd -》 quote -> transfer -> mint
    ...
    function mint(address to) override external lock returns (uint256 liquidity) {
        ...
        //返回铸造费开关 协议费用 按照uniswapV2包含0.05%的费用 可以将其打开或者关闭
