...
contract TokenExchange {
    ...
    mapping(address => address) public WCtoken;
    ...
    function createPool(address token,uint amountTokens) external payable OnlyAdmin {
        ...
        //初始化token
        address WC = address(new FT("WCtoken", "WC"));
        WCtoken[token] = WC;
    ...
    //铸造token
    function mintWC(address token, address to, uint amount0, uint amount1) internal returns (uint liquidity) {

        uint256 _totalSupply = IERC20(WCtoken[token]).totalSupply();
        if (_totalSupply == 0) {
        ...
        FT(WCtoken[token]).mint(to, liquidity);
    }

    //销毁token
    function burnWC(address token, address to) internal returns (uint amount0, uint amount1) {
        uint balance0 = user_token_balance[token][msg.sender];
        uint balance1 = eth_contributed[msg.sender];
        uint liquidity = IERC20(WCtoken[token]).balanceOf(address(msg.sender));

        //把用户wc所占总wc比例的token0和token1返还用户
        uint _totalSupply = IERC20(WCtoken[token]).totalSupply();
        amount0 = liquidity.mul(balance0) / _totalSupply;
        amount1 = liquidity.mul(balance1) / _totalSupply;
        require(amount0 > 0 && amount1 > 0, 'UniswapV2: INSUFFICIENT_LIQUIDITY_BURNED');

        FT(WCtoken[token]).burn(liquidity);
    ...
    function priceETH(address token) public view returns (uint) {
        require(eth_reserves[token] > 0, "eth_reserves must be more than 0!");
    ...
    function addLiquidity(address token, uint minPriceEth, uint maxPriceEth) external payable {
        ...
        //铸造 WC token
        mintWC(token, msg.sender, new_tokens, msg.value);
    ...
    function removeLiquidity(address token, uint amountETH, uint minPriceEth, uint maxPriceEth) public payable {
        ...
        require(amountETH > 0, "ETH amount must be more than 0!");
