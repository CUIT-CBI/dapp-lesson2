...
library Math {
    function getsqrt(uint256 y) internal pure returns (uint256 z) {
        if (y > 3) {
            z = y;
            uint256 x = y / 2 + 1;
            while (x < z) {
...
contract pool is FT{
    ...
    constructor(address _token1, address _token2) FT("liuyangtoken","liuyang") {
        token1 = _token1;
        token2 = _token2;
    }
 ...
 function getAmount(uint256 Inamount,uint256 Inreserve,uint256 Outreserve) private pure returns (uint256 Outamount) {
        Outamount = (Inreserve * Outreserve) / (Inreserve + Inamount);
        return Outamount;
    }
    
    function increaseliquid(uint amount1, uint amount2, address to) external returns(uint liquid) {
       require(to != token1 && to != token2);
       ...
       if(currentSupply == 0 ){
           liquid = Math.getsqrt(amount1 * amount2) - minliqui;
           ...
           liquid = Math.getmin(liquid1,liquid2);
        }
        super._mint(to,liquid);
        reserve1 = uint112(reserveAfter1);
        reserve2 = uint112(reserveAfter2);
    }
    function removeliquid(uint liquid) public returns(uint amount1, uint amount2) {
        (uint112 _reserve1, uint112 _reserve2) = getReserves();
    ...
    function _swap(uint Inamount1, uint Inamount2, uint Minamount,address fromToken, address toToken, address to) external{
