...
contract Swap is FT {
    ...
    function swap(
        ...
        if (amount0out == 0) {
            FT(token0).transferFrom(
                msg.sender,
                address(this),
                FeeCharge(amount1out)
            );
            balance0 -= FeeCharge(amount1out);
            balance0 += amount1out;
            return amount1out;
        } else if (amount1out == 0) {
            FT(token1).transferFrom(
                msg.sender,
                address(this),
                FeeCharge(amount0out)
            );
            balance1 -= FeeCharge(amount0out);
            balance1 += amount0out;
            return amount0out;
        }
    ...
    // 交易费
    function FeeCharge(uint256 total) internal pure returns (uint256) {
