...
contract SSQSwapPair is FT {
    ...
    uint8 public fee; // 万分之几， 比如千分之三就是30
    bool public initialized = false;

    constructor(address _token0, address _token1) FT("SSQSwap LPToken", "LP") {
        token0 = _token0;
        token1 = _token1;
        factory = msg.sender;
    }
    ...
    function addLiquidity(uint256 count) external {
        ...
        require(currentLPTotalSupply > 0, "SSQSwap: Invalid lp token count reserved.");
    ...
    function removeLiquidity(uint256 count) external {
        ...
        uint256 currentLPTotalSupply = totalSupply();
        require(currentLPTotalSupply > count, "SSQSwap: Invalid lp token count to remove.");
        require(balanceOf(account) >= count, "SSQSwap: You don't have enough lp token.");
