...
contract uniswapLZW is ERC20 {
    ...
    //构造交易池
    constructor(address _token0, address _token1) ERC20("LiuZhongWei", "LZW") {
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function addliquidity(uint256 _amount0, uint256 _amount1) public {
        require(_amount0 != 0 && _amount1 != 0, " addliquidity  error");
        uint256 Liquidity;
        if (Reserve0 != 0) {
            uint256 bestReserve = (_amount0 * Reserve0) / Reserve1;
            require(_amount0 > bestReserve, "token less");
            assert(
    ...
    //移除流动性
    function removeLiquiduty(uint256 liquidity) public {
        //收回代币
    ...
    function getAmount(
        ...
        //百分之三手续费
        uint256 _swapAmount = ((997 * (_ReserveOutput * _amount)) / 1000) *
            (_ReserveInput + _amount);
        return _swapAmount;
    }
