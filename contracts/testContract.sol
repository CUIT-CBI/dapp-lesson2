...
contract TestContract {
   ...
   function _update(uint256 balance0,uint256 balance1,uint256 _reserve0,uint256 _reserve1) private {
       ...
       uint32 timeElapsed = blockTimestamp - blockTimestampLast; //计算时间差
  ...
  bool _isDIS = true; //合约功能状态

  uint256 maxPledgeAmount;  //最大质押额度
  uint256 leftMiningAmount; // 剩余额度
  uint256 size;   //质押用户数量

  uint256 totalPledgeAmount;  //已质押总额度
  uint256 startTime;
  uint256 endTime;

  mapping(address => uint256) indexOfMiners;

  struct PledgeOrder {      //质押用户状态
      bool isExist;  
      address ple;
      address tokenT;        //代币类型
      uint256 tokenAmount;   //质押额度
      uint256 profitToken;   //收益
      uint256 lastBlock;      //最近一次提取收益的时间
      uint256 index;
