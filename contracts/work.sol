...
struct AddLiquidityParams {
    ...
    int24 tickLower;    // 流动性的价格下限（以 tokenA 计价），这里传入的是 tick index
    int24 tickUpper;    // 流动性的价格上线（以 tokenA 计价），这里传入的是 tick index
    uint128 amount;     // 流动性 L 的值
    uint256 amount0Max; // 提供的 tokenA 上限数
    uint256 amount1Max; // 提供的 tokenB 上限数
}
...
Slot0 memory _slot0 = slot0; // SLOAD for gas optimization
    ...
    function tokenAToTokenB(uint256 _inputAmount, uint256 _minTokens) public {
        ...
        reserve0 = ERC20(tokenA).balanceOf(address(this));
        reserve1 = ERC20(tokenB).balanceOf(address(this));   //更新余额
    }    
