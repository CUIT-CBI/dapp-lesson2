...
contract QykSwapPair is FT {
    ...
    constructor(address _token0, address _token1) FT("QykSwap LPToken", "LP") {
        token0 = _token0;
        token1 = _token1;
        factory = msg.sender;
    }
    ...
    modifier onlyFactory() {
        require(msg.sender == factory, "QykSwap: only factory!");
        _;
    }
    ...
    function addLiquidity(uint256 count) external {
        ...
        require(currentLPTotalSupply > 0, "QykSwap: Invalid");
    ...
    function removeLiquidity(uint256 count) external {
        ...
        uint256 currentLPTotalSupply = totalSupply();
        require(currentLPTotalSupply > count, "QykSwap: Invalid");
        require(balanceOf(account) >= count, "QykSwap: You don't have enough lp token.");
    ...
    function swap(uint256 _amount0In, uint256 _amount1In, uint256 _amount0OutMin, uint256 _amount1OutMin) external {
        ...
        require(amount0Out >= _amount0OutMin && amount1Out >= _amount1OutMin, "QykSwap: price too low");
    ...
    function getAmountOut(uint256 _amount0In, uint256 _amount1In) 
        public
        view
